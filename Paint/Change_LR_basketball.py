...
class LogisticRegression(object):
    ...
    def load_data(self):
        ...
        self.train_set_x=numpy.load(self.dataset+"train_set_x0-1.npy")
        self.valid_set_x=numpy.load(self.dataset+"valid_set_x0-1.npy")
        self.test_set_x = numpy.load(self.dataset+"test_set_x0-1.npy")

        # print(self.test_set_x[0])

        self.train_set_y=numpy.load(self.dataset+"train_set_y.npy")
        self.valid_set_y=numpy.load(self.dataset+"valid_set_y.npy")
        self.test_set_y=numpy.load(self.dataset+"test_set_y.npy")
...
def sgd_optimization_mnist(learning_rate=0.13,
    ...
    while (epoch < n_epochs) and (not done_looping):
        ...
        for minibatch_index in range(classifier.n_train_batches):#每次循环完成训练所有样本
            ...
            if (iter + 1) % validation_frequency == 0:#每frequency个batch验证一次，这是不确定的一个数，这里为n_tarin_batches就相当于所有样本训练完检测一次，此时恰好完成一个epoch
